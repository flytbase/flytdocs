...
Connect ground, power and signal pins of the ESCs in the respective PWM output channels. 
...
6. Programmable LEDs
^^^^^^^^^^^^^^^^^^^^
LEDs available are user-programmable.
...
14. Micro SD
^^^^^^^^^^^^
32 GB Micro SD card which consists of the FlytOS image.
...
16. VI Sensor
^^^^^^^^^^^^^
Monitors battery status and issues low battery warnings, available flight time left.
